...
const mainRegex = /<div className="w-full flex-1 flex flex-col items-center justify-center">([\s\S]*?)<\/div>\s*<\/main>/;
const match = content.match(mainRegex);
if (match) {
  const swiperBody = `<div className="w-full flex-1 flex flex-col items-center justify-center">
        <Swiper
          effect={'cards'}
          grabCursor={true}
  ...
  content = content.replace(match[0], swiperBody);
  fs.writeFileSync('src/py_builder.tsx', content);
  console.log("Transformed py_builder.tsx using Regex targeting </main>");
} else {
  console.log("Regex did not match in py_builder");
}
